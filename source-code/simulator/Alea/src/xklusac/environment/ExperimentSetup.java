...
import xklusac.algorithms.queue_based.multi_queue.FCFSRescheduling;
...
public class ExperimentSetup {
    ...
    private static String resultDirName = "";
    
    public static double reschedulingDelay = 1000;
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 1){
            resultDirName = args[1].split(";")[0].split("\\.")[0] + "." + getDate();
        }
        if (args.length > 2){
            if (args[2].equals("-t")){
                visualize = false;
        ...
        if (args.length > 3){
            reschedulingDelay = Double.parseDouble(args[3]);
        }
        ...
        String data_sets[];
        data_sets = aCfg.getStringArray("data_sets");
        ...
        if (resultDirName == null || resultDirName == ""){
           dir[1] = getDate(); 
        } else {
            String[] f = resultDirName.split("/");
        ...
        String fileName = aCfg.getFileName();
        if (fileName.startsWith(".")){
            String[] f = fileName.split("/");
            fileName = f[f.length-1];
        }
        File destinationF = new File(ExperimentSetup.getDir(DirectoryLevel.EXPERIMENT_ROOT) + File.separator + fileName);
        ...
        for (int set = 0; set < data_sets.length; set++) {
            ...
            for (int sel_alg = 0; sel_alg < algorithms.length; sel_alg++) {
                ...
                if (alg == 800){
                    policy = new FCFSRescheduling(scheduler);
                    suff = "FCFSRescheduling";
                    if (resource_spec_packing) {
                        suff += "-pack";
                    }
                }
                ...
                for (int pass_count = 1; pass_count <= experiment_count; pass_count++) {
                    ...
                    try {
                        ...
                        // copy machines file and rename as same as data file
                        if (args.length >= 2){
                            String machineDatasets[] = args[1].split(";");
                            if (machineDatasets.length != data_sets.length){
                                throw new Exception("Number of data files and machine files is not same.");
                            }
                            for (int i = 0; i < machineDatasets.length; i++){
                                if (!machineDatasets[i].equals(data_sets[i] + ".machines")){
                                    Files.copy(Paths.get(ExperimentSetup.data_sets + "/" + machineDatasets[i]), 
                                        Paths.get(ExperimentSetup.data_sets + "/" + data_sets[i] + ".machines"),
