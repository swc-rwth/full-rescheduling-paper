...
import addon.rescheduling.AddOnTags;
...
public class Scheduler extends GridSim {
    ...
    int periodInterval = 600;
    int periodCount = 0;
    
    double prevEventClock = 0.0;
    double eventClockDuplicateAdjuster = 0.0;
    ArrayList<String> prevEventOutputLines = new ArrayList<String>();
    ...
    public void body() {
        ...
        while (true) {
            ...
            if (resList.size() > 0) {
                ...
                //if (visualize) {
                    ...
                    super.sim_schedule(this.getEntityId(this.getEntityName()), 0.0, AddOnTags.DETAIL_SCHEDULER_COLLECT);
                //}
        ...
        while (!end_of_submission || received < in_job_counter) {
            ...
            // peroidically log detail resource usage
            if (ev.get_tag() == AddOnTags.DETAIL_SCHEDULER_COLLECT){
                // Detail hour usage
                try {
                    for (int c = 0; c < totalResource; c++) {
                        String name = ((ResourceInfo) resourceInfoList.get(c)).resource.getResourceName();
                        double usage = (CommonObjectives.getClusterUsage(c) * 100) / 100.0;
                        out.writeString(folder_prefix + "/detail_period_usage_" + suff + ".csv", name + "\t" + periodCount + "\t" + usage);
                    }
                } catch (IOException ex) {
                    ex.printStackTrace();
                }
                periodCount += periodInterval;
                // send periodical event that will arrive in periodInterval
                super.sim_schedule(this.getEntityId(this.getEntityName()), periodInterval, AddOnTags.DETAIL_SCHEDULER_COLLECT);
                continue;
            }
            ...
            if (ev.get_tag() == AleaSimTags.GRIDLET_STARTED) {
                ...
                Debugger.debug("GRIDLET_STARTED " + gl.getGridletID());
            ...
            if (ev.get_tag() == GridSimTags.GRIDLET_RETURN) {
                ...
                Debugger.debug(String.format("GRIDLET_RETURN %d", gridlet_received.getGridletID()));
            ...
            if (ev.get_tag() == AleaSimTags.GRIDLET_INFO) {
                ...
                Debugger.debug("GRIDLET_INFO " + gl.getGridletID());
    ...
    /**
     * Log resource usage stats when job started or finished
     * If used, it slows down the simulation
     */
    private void logUsageOnEvent(){
        double ck = GridSim.clock();
        if ((ck - prevEventClock) < 1){
            // skip event that happens too close with each other
            prevEventOutputLines.clear();
            for (int c = 0; c < totalResource; c++) {
                String name = ((ResourceInfo) resourceInfoList.get(c)).resource.getResourceName();
                double usage = (CommonObjectives.getClusterUsage(c) * 100) / 100.0;
                prevEventOutputLines.add(name + "\t" + ck + "\t" + usage);
            }
            prevEventClock = ck;
            return;
        }
        prevEventClock = ck;
        try {
            for (int i = 0; i < prevEventOutputLines.size(); i++){
                out.writeString(folder_prefix + "/detail_event_usage_" + suff + ".csv", prevEventOutputLines.get(i));
            }
            prevEventOutputLines.clear();
        } catch (IOException ex) {
            ex.printStackTrace();
    ...
    public void submitJob(ComplexGridlet gl, int resID) {
        ...
        boolean r = gridletSubmit(gl, resID);
