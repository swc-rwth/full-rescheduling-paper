...
class AdvancedSpaceShared extends AllocPolicy {
    ...
    public void body() {
        ...
        while (Sim_system.running()) {
            ...
            if (ev.get_src() == super.myId_ && gridletInExecList_.size() > 0) {
                Debugger.debug(String.format("InternalEvent of Policy %d, Gridlet %d", super.myId_, gridletInExecList_.get(0).getGridletID()));
    ...
    public void gridletSubmit(Gridlet gl, boolean ack) {
        ...
        for (ResGridlet rgl : gridletInExecList_){
            if (rgl.getGridlet() == gl){
                rgl.getGridlet().setGridletLength(gl.getGridletLength());
                super.sendInternalEvent(0);
                return;
            }
        }
